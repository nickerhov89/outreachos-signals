"""OutreachOS Signals — FastAPI web app (v2, plain Jinja2).
Reads from data/signals.db, serves dashboard, niche pages, signal detail, leads, XLSX export.
Run: uvicorn web.app_v2:app --host 0.0.0.0 --port 8000 --workers 1"""
import io
import json
import sqlite3
...
# Plain Jinja2 (avoid Starlette/FastAPI template cache bug)
env = Environment(
...
SOURCE_COLORS = {
    "greenhouse": "#22c55e", "lever": "#3b82f6", "apify_threads": "#a855f7",
    "funding_news": "#f59e0b", "github": "#1f2937", "linkedin_change": "#0a66c2",
    "reddit": "#ff4500", "hackernews": "#ff6600", "podcast": "#ec4899",
    "g2": "#ff492c", "job_text_tech": "#06b6d4",
}
...
def render(name: str, **ctx) -> HTMLResponse:
    """Render a template with common context. Plain Jinja2, no Starlette wrapper."""
    base_ctx = {
        "request": None,
        "range_niches": NICHES,
    ...
    base_ctx.update(ctx)
    try:
        html = env.get_template(name).render(**base_ctx)
    ...
    with get_db() as c:
        ...
        for n in NICHES:
            row = c.execute("""
                ...
                FROM signal_classifications cl WHERE cl.niche_id = ?
            """, (n["id"],)).fetchone()
            events = c.execute("""
            ...
            niche_stats.append({**n, "event_count": events, "classified_count": row["classified"]})
        sources = c.execute("""
            SELECT source, COUNT(*) as c FROM signal_events
        ...
        recent = c.execute("""
            ...
            WHERE cl.score >= 5
            ORDER BY cl.score DESC, e.collected_at DESC LIMIT 10
        """).fetchall()
    return render("dashboard.html", request=request, kpis=kpis, niche_stats=niche_stats,
                  sources=[dict(r) for r in sources], recent=[dict(r) for r in recent])
    ...
    with get_db() as c:
        if min_score > 0:
            rows = c.execute("""
                ...
                ORDER BY cl.score DESC, e.collected_at DESC LIMIT 200
            ...
            rows = c.execute("""
                ...
                ORDER BY e.collected_at DESC LIMIT 200
            """, (niche_id, f"%{niche['name'].split()[0]}%")).fetchall()
    return render("niche.html", request=request, niche=niche, signals=[dict(r) for r in rows], min_score=min_score)
...
async def signal_detail(request: Request, event_id: str):
    ...
    return render("signal.html", request=request, signal=sig)
...
async def leads_page(request: Request, min_score: float = 7.0):
    with get_db() as c:
        rows = c.execute("""
            ...
            JOIN signal_classifications cl ON cl.event_id = e.event_id
            WHERE cl.score >= ?
            ORDER BY cl.score DESC, e.collected_at DESC LIMIT 100
        """, (min_score,)).fetchall()
    leads = []
    for r in rows:
        d = dict(r)
        if d.get("buyer_contacts_json"):
            ...
            d["buyer_contacts"] = []
        leads.append(d)
    return render("leads.html", request=request, leads=leads, min_score=min_score)
...
async def api_signals(
    ...
    query += " ORDER BY cl.score DESC, e.collected_at DESC LIMIT ?"
...
async def export_xlsx(niche: int | None = None, min_score: float = 0):
    ...
    query += " ORDER BY cl.score DESC LIMIT 5000"
    with get_db() as c:
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("web.app_v2:app", host="0.0.0.0", port=8000, reload=False)
